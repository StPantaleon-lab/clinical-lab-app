...
function LabInput({ refKey, values, sex, onChange }) {
  ...
  return (
    <div style={{
      border: `2px solid ${st ? st.border : '#e2e8f0'}`,
      borderRadius: 10,
      padding: '9px 11px',
      background: st ? st.bg : 'white',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
        ...
        {ev && (
          <span style={{
            ...
            background: 'rgba(255,255,255,0.7)', color: st.text,
          }}>
            {st.badge}
          </span>
        )}
...
export default function LabInputPanel({ values, sex, setSex, onChange, onClear }) {
  ...
  const abnormal = entered.filter((k) => {
    const ev = evalVal(k, values[k], sex);
    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {['male', 'female'].map((s) => (
            <button key={s} onClick={() => setSex(s)} style={{
              ...
              border: `2px solid ${sex === s ? '#3b82f6' : '#e2e8f0'}`,
              background: sex === s ? '#eff6ff' : 'white',
              color: sex === s ? '#1d4ed8' : '#64748b',
        ...
        <div style={{ fontSize: 12, color: '#64748b' }}>
          入力済み <strong style={{ color: '#0f172a' }}>{entered.length}</strong> 件　|　
          異常値 <strong style={{ color: abnormal.length > 0 ? '#dc2626' : '#16a34a' }}>{abnormal.length}</strong> 件
        </div>
        {entered.length > 0 && (
          <button
            onClick={onClear}
      ...
      {groups.map(({ group, keys }) => (
        <details key={group} open style={{ marginBottom: 10 }}>
          <summary style={{
            ...
            cursor: 'pointer', fontWeight: 700, fontSize: 13, color: '#374151',
          ...
          <div style={{
            ...
            {keys.map((k) => (
              <LabInput key={k} refKey={k} values={values} sex={sex} onChange={onChange} />
            ))}
          </div>
        </details>
      ))}
