      ...
      <header style={{
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 0 0' }}>
            <div style={{
              background: '#3b82f6', borderRadius: 10, width: 38, height: 38,
              display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20,
            }}>🔬</div>
            <div>
              <h1 style={{ margin: 0, fontSize: 18, fontWeight: 800, letterSpacing: '-0.3px' }}>
                臨床検査値 疾患推定ツール
              </h1>
              <p style={{ margin: 0, fontSize: 11, color: '#94a3b8' }}>
                正常範囲に基づく古典的判定 ／ 症候ブランチ ／ 検査カバレッジ分析
              </p>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 2, marginTop: 12 }}>
            {TABS.map((t) => {
              const badge =
                t.id === 'result' && matchedDiseases.length > 0
      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '20px 16px 40px' }}>
        ...
        {entered.length > 0 && (
          <div style={{
            ...
            <span>判定可能疾患: <strong style={{ color: '#16a34a' }}>{DISEASES.filter(d => d.requiredKeys.every(k => entered.includes(k))).length}</strong>件</span>
            <span>陽性候補: <strong style={{ color: matchedDiseases.length > 0 ? '#dc2626' : '#16a34a' }}>{matchedDiseases.length}</strong>件</span>
            <span>あと少しで判定可能: <strong style={{ color: '#d97706' }}>{partial.length}</strong>件</span>
            {tab !== 'result' && matchedDiseases.length > 0 && (
              <button
