...
export const DISEASES = [
  ...
  // ═══════════════════════════════════════════════
  {
    ...
    requiredKeys: [
      { key: "Alb",  direction: "low" },
      { key: "UPCR", direction: "high" },
      { key: "TC",   direction: "high" },
      { key: "Cre",  direction: "any" },
    ],
    conditionFn: (v, ev) => ev.Alb === "low" && (ev.UPCR === "high" || ev.TC === "high"),
    symptoms: [
      { key: "leg_edema",      label: "下腿浮腫（両側性・圧痕性）" },
      { key: "proteinuria",    label: "大量蛋白尿（≥3.5g/日）" },
      { key: "weight_gain",    label: "体重増加" },
    ],
    note: "成人ネフローゼ症候群の最多原因（約40%）。特発性（PLA2R抗体陽性）と続発性（悪性腫瘍・薬剤・SLE）がある。血尿は少ない。腎生検でスパイク形成・IgG顆粒状沈着が特徴。",
    ruleOutKeys: ["CH50", "C3", "C4"],
    ruleOutNote: "補体正常→SLEによる続発性膜性腎症（クラスV）を否定。抗PLA2R抗体で特発性を確定。",
  },
  {
    id: "minimal_change",
    name: "微小変化型ネフローゼ症候群（MCNS）",
    category: "糸球体疾患",
    requiredKeys: [
      { key: "Alb",  direction: "low" },
      { key: "UPCR", direction: "high" },
    ...
    conditionFn: (v, ev) => ev.Alb === "low" && (ev.UPCR === "high" || ev.TC === "high"),
    symptoms: [
      { key: "leg_edema",      label: "急速に出現した浮腫" },
      { key: "proteinuria",    label: "大量蛋白尿（選択性高い）" },
      { key: "weight_gain",    label: "急激な体重増加" },
    ],
    note: "小児ネフローゼ症候群の最多原因。成人でも約20%。血尿・高血圧は少ない。ステロイドが著効する。腎生検でほぼ正常（電顕で足突起消失）。",
    ruleOutKeys: ["URBCh", "CH50"],
    ruleOutNote: "血尿（赤血球尿）があれば膜性増殖性GNやIgA腎症を考える。補体正常が特徴。",
  },
  {
    id: "focal_segmental",
    name: "巣状分節性糸球体硬化症（FSGS）",
    category: "糸球体疾患",
    requiredKeys: [
      { key: "Alb",  direction: "low" },
      { key: "UPCR", direction: "high" },
    ...
    conditionFn: (v, ev) => ev.Alb === "low" && ev.UPCR === "high" && ev.Cre === "high",
    symptoms: [
      { key: "leg_edema",   label: "浮腫" },
      { key: "proteinuria", label: "蛋白尿（非選択性）" },
      { key: "hematuria",   label: "血尿（しばしば合併）" },
      { key: "hypertension",label: "高血圧" },
    ],
    note: "成人ネフローゼ症候群の約20%。腎機能低下を伴うことが多い。特発性・続発性（HIV・肥満・鎌状赤血球症）がある。ステロイド抵抗性が多い。",
    ruleOutKeys: [],
    ruleOutNote: "腎生検なしでは確定診断不可。ステロイド治療反応性でMCNSと鑑別。",
  },
  {
    ...
    requiredKeys: [
      { key: "HbA1c", direction: "high" },
      { key: "UAlb",  direction: "high" },
      { key: "eGFR",  direction: "low" },
    ],
    conditionFn: (v, ev) => (ev.HbA1c === "high" || ev.Glu === "high") && (ev.UAlb === "high" || ev.UPCR === "high"),
    symptoms: [
      { key: "leg_edema",    label: "浮腫" },
      { key: "proteinuria",  label: "蛋白尿（微量アルブミン尿→顕性）" },
      { key: "visual_change",label: "糖尿病網膜症（合併）" },
      { key: "neuropathy",   label: "糖尿病性末梢神経障害（合併）" },
      { key: "dialysis",     label: "透析に向かう進行性腎不全" },
    ],
    note: "糖尿病の三大合併症の一つ。微量アルブミン尿（30〜300mg/gCr）が早期指標。網膜症・神経障害との三徴。SGLT2阻害薬が腎保護効果。",
    ruleOutKeys: ["Glu", "HbA1c"],
    ruleOutNote: "Glu・HbA1cが正常なら糖尿病性腎症は否定できる。",
  },
  {
    id: "lupus_nephritis",
    name: "ループス腎炎",
    ...
    conditionFn: (v, ev) => ev.Alb === "low" && ev.CH50 === "low" && ev.C3 === "low",
    symptoms: [
      { key: "proteinuria",  label: "蛋白尿・血尿" },
      { key: "malar_rash",   label: "蝶形紅斑" },
      { key: "photo_sens",   label: "光線過敏症" },
      { key: "arthritis",    label: "関節痛・関節炎" },
      { key: "leg_edema",    label: "浮腫（ネフローゼ型）" },
    ],
    note: "SLEに合併する腎炎（ISN/RPS分類 I〜VI型）。補体低下（CH50・C3・C4↓）＋抗dsDNA抗体↑が特徴。クラスIIIとIVが最も重症。免疫抑制療法が必要。",
    ruleOutKeys: ["C4"],
    ruleOutNote: "C4正常なら古典的経路の活性化が少なく、ループス腎炎より膜性腎症（クラスV）を考える。",
  },
  {
    id: "mpgn",
    name: "膜性増殖性糸球体腎炎（MPGN）",
    category: "糸球体疾患",
    requiredKeys: [
      { key: "C3",    direction: "low" },
      { key: "Cre",  direction: "high" },
    ...
    conditionFn: (v, ev) => ev.C3 === "low" && (ev.Cre === "high" || ev.UPCR === "high"),
    symptoms: [
      { key: "hematuria",    label: "血尿（肉眼的または顕微鏡的）" },
      { key: "proteinuria",  label: "蛋白尿（混合型：ネフローゼ＋腎炎）" },
      { key: "hypertension", label: "高血圧" },
      { key: "leg_edema",    label: "浮腫" },
    ],
    note: "C3著明低下（C4は正常）が特徴。C3ネフリティック因子（C3Nef）が第二経路を持続活性化。ネフローゼ症候群と腎炎症候群の混合型の呈することが多い。",
    ruleOutKeys: ["C4"],
    ruleOutNote: "C4も低下なら古典的経路の活性化（免疫複合体型）を考える。C4正常+C3低下は第二経路活性化（C3腎症）を示唆。",
  },
  {
    id: "poststrep_gn",
    name: "急性糸球体腎炎（連鎖球菌感染後）",
    category: "糸球体疾患",
    requiredKeys: [
      { key: "CH50", direction: "low" },
      { key: "C3",   direction: "low" },
    ...
    conditionFn: (v, ev) => ev.CH50 === "low" && ev.C3 === "low" && ev.Cre === "high",
    symptoms: [
      { key: "hematuria",    label: "血尿（コーラ色尿・肉眼的血尿）" },
      { key: "leg_edema",    label: "浮腫（急性に出現）" },
      { key: "hypertension", label: "高血圧" },
      { key: "oliguria",     label: "乏尿" },
    ],
    note: "A群β溶連菌感染後1〜3週で発症。補体低下（CH50・C3↓、C4正常）＋血尿＋浮腫＋高血圧の四徴。ASO・ASK上昇。多くは自然寛解。",
    ruleOutKeys: [],
    ruleOutNote: "ASO・ASKの上昇で連鎖球菌感染を確認。補体は6〜8週で正常化（持続低下ならMPGNやループス腎炎を鑑別）。",
  },
  {
    id: "amyloid_nephropathy",
    name: "腎アミロイドーシス",
    category: "糸球体疾患",
    requiredKeys: [
      { key: "Alb",  direction: "low" },
    ...
    conditionFn: (v, ev) => ev.Alb === "low" && ev.UPCR === "high" && ev.TP === "low",
    symptoms: [
      { key: "leg_edema",    label: "浮腫（大量蛋白尿・ネフローゼ）" },
      { key: "proteinuria",  label: "大量蛋白尿" },
      { key: "weight_loss",  label: "体重減少（全身性アミロイドーシス）" },
      { key: "bone_pain",    label: "骨髄腫関連（AL型）" },
    ],
    note: "ALアミロイドーシス（多発性骨髄腫・原発性）またはAAアミロイドーシス（慢性炎症・RA）。Congo-Red染色陽性・偏光顕微鏡でリンゴ緑色複屈折が特徴。Congo-Red陰性なら否定的。",
    ruleOutKeys: ["IgG", "IgA", "IgM"],
    ruleOutNote: "M蛋白（IgG/A/M単クローン性増加）の有無でALアミロイドーシスを疑う。Congo-Red染色陰性なら腎アミロイドーシスは否定できる。",
  },
];

export const CATEGORIES = [
  "貧血", "出血性疾患", "肝・胆・膵疾患", "腎疾患", "糸球体疾患",
